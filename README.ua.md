...
Кожен інструмент приймає параметр `impersonate` — ім'я користувача бази даних для імперсоналізації:
...
Усі інструменти приймають `server`, `username?`, `password?` та `impersonate`.
