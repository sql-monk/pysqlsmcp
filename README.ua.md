...
Кожна сесія запитів починається з примусового преамбулу (`_SET_PREAMBLE` у `isqls.py`):
...
Імперсоналізація — це основний механізм безпеки. Логін, що підключається (Windows auth або SQL auth), **ніколи** не є ідентичністю, під якою виконуються запити користувачів. Замість цього `isqls` негайно перемикає контекст безпеки після SET-преамбулу.
...
isqls.py            — З'єднання, SET-преамбул, EXECUTE AS, виконання запитів
deploy/
